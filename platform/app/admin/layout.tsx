...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  const supabase=await createClient();const [surveyResult,archivedResult,sessionResult]=await Promise.all([supabase.from('surveys').select('id,name,status,responses(count)').neq('status','archived').order('updated_at',{ascending:false}),supabase.from('surveys').select('id',{count:'exact',head:true}).eq('status','archived'),supabase.from('builder_sessions').select('id').eq('status','in_progress')]);const surveys=surveyResult.data??[],sessionCount=sessionResult.data?.length??0;const counts={inProgress:sessionCount,draft:surveys.filter(item=>item.status==='draft'||item.status==='unpublished').length,published:surveys.filter(item=>item.status==='published').length,withResponses:surveys.filter(item=>(item.responses?.[0]?.count??0)>0).length,archived:archivedResult.count??0};
