...
export default async function AdminLayout({children}:{children:React.ReactNode}){
  ...
  if(!profile.is_active)return <main className="shell" style={{maxWidth:640,padding:'10vh 0'}}><section className="card stack" style={{padding:28}}><div><p className="muted">Questionnaire Platform</p><h1>アカウント利用停止中</h1></div><p>管理者にお問い合わせください。</p><form action={logoutAction}><button className="btn secondary" type="submit">ログアウト</button></form></section></main>;
  const [allSurveys,sessions]=await Promise.all([getAdminSurveys(),getAdminBuilderSessions()]);const surveys=allSurveys.filter(item=>item.status!=='archived');const counts={inProgress:sessions.length,draft:surveys.filter(item=>item.status==='draft'||item.status==='unpublished').length,published:surveys.filter(item=>item.status==='published').length,withResponses:surveys.filter(item=>(item.responses?.[0]?.count??0)>0).length,archived:allSurveys.filter(item=>item.status==='archived').length};
