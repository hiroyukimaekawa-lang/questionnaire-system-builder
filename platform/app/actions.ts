...
import { createClient } from '@/lib/supabase/server'; import { defaultConfig, normalizeQuestionFontSize, slugify, validateQuestion, validateSlug } from '@/lib/survey'; import type { BuilderContext, SurveyQuestion } from '@/types/database';
import { ruleBasedBuilderEngine } from '@/lib/builder/engine';
import { getThemeTemplate, themeIdForBusiness } from '@/lib/theme/templates';
import {completionSettingsConfig,mergeSurveyConfig} from '@/lib/config';
import {canDemoteAdmin} from '@/lib/auth/security';
import {parseCompletionRulesJson,validateRulesForQuestions} from '@/lib/completion-validation';
...
export async function createSurveyAction(_:unknown,form:FormData){
  ...
  try{
    ...
    const {data:v,error:ve}=await s.from('survey_versions').insert({survey_id:surveyId,version:1,status:'draft',config,created_by:user.id}).select('id').single();
    if(ve||!v)throw ve;
...
export async function publishAction(id:string){
  try{
    const {s}=await staff();
    const {data:publishedId,error}=await s.rpc('publish_survey',{p_survey_id:id});
    ...
    const [{data:published,error:pe},{data:draft,error:de}]=await Promise.all([
      s.from('survey_versions').select('config,questions(id,sort_order)').eq('id',publishedId).eq('survey_id',id).single(),
      s.from('survey_versions').select('config,questions(id,sort_order)').eq('id',survey.current_draft_version_id).eq('survey_id',id).single()
    ]);
    if(pe||de||!published||!draft)return {error:'公開後の下書きを確認できませんでした。管理画面で状態を確認してください。'};
    const oldByOrder=new Map((published.questions??[]).map((q:any)=>[q.sort_order,q.id]));
    const idMap=Object.fromEntries((draft.questions??[]).map((q:any)=>[oldByOrder.get(q.sort_order),q.id]).filter(([old])=>old));
    const {data:updated,error:ue}=await s.from('survey_versions').update({config:remapConfigQuestions(draft.config,idMap)}).eq('id',survey.current_draft_version_id).eq('status','draft').select('id').single();
    if(ue||!updated)return {error:'公開後の下書き設定を確認できませんでした。管理画面で状態を確認してください。'};
    revalidatePath('/admin');
    revalidatePath(`/admin/surveys/${id}`);
...
export async function duplicateSurveyAction(id:string,_:unknown,form:FormData){
  try{
    const {s,user}=await staff();
    const {data:src}=await s.from('surveys').select('*,survey_versions!surveys_current_draft_version_id_fkey(*,questions(*,question_options(*)))').eq('id',id).neq('status','archived').single();
    ...
    const idMap=Object.fromEntries(ordered.map((q:any)=>[q.id,crypto.randomUUID()]));
    const copiedConfig=remapConfigQuestions({...source.config,title:`${name} お客様アンケート`},idMap);
    const {data:v}=await s.from('survey_versions').insert({survey_id:n.id,version:1,status:'draft',config:copiedConfig,created_by:user.id}).select('id').single();
    for(const q of ordered){
      const {data:nq}=await s.from('questions').insert({id:idMap[q.id],survey_version_id:v!.id,type:q.type,title:q.title,description:q.description,required:q.required,sort_order:q.sort_order,settings:q.settings}).select('id').single();
...
export async function updateRoleAction(profileId:string,_:unknown,form:FormData){const {s,p}=await staff();if(p.role!=='admin')return {error:'管理者のみ変更できます。'};const role=String(form.get('role'));if(role!=='admin'&&role!=='sales')return {error:'権限が不正です。'};const [{data:target},{count}]=await Promise.all([s.from('profiles').select('role').eq('id',profileId).single(),s.from('profiles').select('id',{count:'exact',head:true}).eq('role','admin')]);if(!target)return {error:'対象ユーザーが見つかりません。'};if(!canDemoteAdmin(target.role,role,count??0))return {error:'最後の管理者は営業権限へ変更できません。'};const {error}=await s.from('profiles').update({role}).eq('id',profileId);if(error)return {error:'権限を更新できませんでした。'};revalidatePath('/admin/users');return {success:'権限を更新しました。'};}

export async function approveUserAction(profileId:string){try{const {s,p}=await staff();if(p.role!=='admin')return {error:'管理者のみ承認できます。'};const {data,error}=await s.from('profiles').update({is_active:true}).eq('id',profileId).eq('is_active',false).select('id').maybeSingle();if(error||!data)return {error:'対象ユーザーを承認できませんでした。'};revalidatePath('/admin/users');return {success:'利用を許可しました。'};}catch(e){return {error:e instanceof Error?e.message:'対象ユーザーを承認できませんでした。'};}}

export async function deactivateUserAction(profileId:string){try{const {s,user,p}=await staff();if(p.role!=='admin')return {error:'管理者のみ実行できます。'};if(profileId===user.id)return {error:'自分自身は利用停止できません。'};const [{data:target},{count}]=await Promise.all([s.from('profiles').select('role').eq('id',profileId).single(),s.from('profiles').select('id',{count:'exact',head:true}).eq('role','admin').eq('is_active',true)]);if(!target)return {error:'対象ユーザーが見つかりません。'};if(!canDemoteAdmin(target.role,'sales',count??0))return {error:'最後の管理者は利用停止できません。'};const {data,error}=await s.from('profiles').update({is_active:false}).eq('id',profileId).eq('is_active',true).select('id').maybeSingle();if(error||!data)return {error:'対象ユーザーを利用停止できませんでした。'};revalidatePath('/admin/users');return {success:'利用を停止しました。'};}catch(e){return {error:e instanceof Error?e.message:'対象ユーザーを利用停止できませんでした。'};}}

export async function sendPasswordResetAction(profileId:string){try{const {s,p}=await staff();if(p.role!=='admin')return {error:'管理者のみ実行できます。'};const {data:target}=await s.from('profiles').select('email').eq('id',profileId).single();if(!target?.email)return {error:'対象ユーザーを確認できませんでした。'};const {error}=await s.auth.resetPasswordForEmail(target.email,{redirectTo:`${appUrl()}/auth/confirm?next=/admin/account/update-password`});if(error)return {error:'再設定メールを送信できませんでした。'};return {success:'アカウントが存在する場合、再設定メールを送信しました。'};}catch{return {error:'再設定メールを送信できませんでした。'};}}
...
export async function saveCompletionSettingsAction(surveyId:string,versionId:string,_:unknown,form:FormData){try{const {s}=await staff();if(!await isActiveSurvey(s,surveyId))return {error:'アンケートを更新できませんでした。'};const reviewUrl=String(form.get('googleReviewUrl')||'').trim(),reviewMode=form.get('googleReviewMode')==='all'?'all':'disabled';if(reviewUrl)try{new URL(reviewUrl)}catch{return {error:'Google口コミURLが不正です。'}};const rules=parseCompletionRulesJson(String(form.get('completionRules')||'[]'));const [{data},{data:rows}]=await Promise.all([s.from('survey_versions').select('config').eq('id',versionId).eq('survey_id',surveyId).eq('status','draft').single(),s.from('questions').select('id,type,title,description,required,sort_order,settings,question_options(id,label,value,sort_order)').eq('survey_version_id',versionId)]);if(!data)return {error:'下書きが見つかりません。'};const questions=(rows??[]).map((q:any)=>({id:q.id,type:q.type,title:q.title,description:q.description,required:q.required,sortOrder:q.sort_order,settings:q.settings??{},options:(q.question_options??[]).map((o:any)=>({id:o.id,label:o.label,value:o.value,sortOrder:o.sort_order}))}));const validation=validateRulesForQuestions(rules,questions);if(validation)return {error:validation};if(reviewMode==='all'&&!reviewUrl)return {error:'Google口コミURLを入力してください。'};const config=completionSettingsConfig(data.config,reviewMode,reviewUrl,rules);const {error}=await s.from('survey_versions').update({config}).eq('id',versionId).eq('survey_id',surveyId).eq('status','draft');if(error)throw error;revalidatePath(`/admin/surveys/${surveyId}`);return {success:'回答後設定を保存しました。'}}catch(e){return {error:e instanceof Error?e.message:'回答後設定を保存できませんでした。'}}}
