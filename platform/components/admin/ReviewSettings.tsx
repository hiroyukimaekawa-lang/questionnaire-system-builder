...
function perQuestionRule(scores:SurveyQuestion[],current:GoogleReviewRule):GoogleReviewRule{return {logic:'or',conditions:scores.map(q=>{const existing=current.conditions.find(c=>c.questionId===q.id&&c.operator==='gte');return {questionId:q.id,operator:'gte',value:existing?Math.min(Math.max(existing.value,1),scoreMax(q)):defaultThreshold(q)};})};}
...
function ruleNeedsNormalization(rule:GoogleReviewRule,scores:SurveyQuestion[]){
  ...
  const malformed=rule.conditions.length!==scores.length||new Set(conditionIds).size!==conditionIds.length||rule.conditions.some(c=>c.operator!=='gte'||!scoreIds.has(c.questionId))||scores.some(q=>!conditionIds.includes(q.id));
  if(malformed)return true;
  return rule.logic==='and'&&rule.conditions.length>0&&commonThreshold(rule,scores)===null;
}
...
export function ReviewSettings({config,questions,onChange}:{config:SurveyConfig;questions:SurveyQuestion[];onChange:(patch:Partial<SurveyConfig>)=>void}) {
  ...
  const [conditionStyle,setConditionStyle]=useState<'all'|'per-question'>(effectiveRule.logic==='or'?'per-question':'all');
  ...
  const displayedCommonThreshold=detectedCommon??Math.min(effectiveRule.conditions[0]?.value??9,maxScale);

  useEffect(()=>{
    if(!needsNormalization)return;
    const normalized=perQuestionRule(scores,rule);
    setConditionStyle('per-question');
  ...
  const changeMode=(value:'disabled'|'all'|'score')=>{
    if(value==='score'&&scores.length){
      ...
      setConditionStyle(next.logic==='or'?'per-question':'all');
  ...
  const changePerQuestionThreshold=(questionId:string,value:number)=>{
    ...
    onChange({googleReviewRule:{logic:'or',conditions:normalized.conditions.map(c=>c.questionId===questionId?{...c,value}:c)}});
  };
  ...
  return <div className="stack review-settings"><fieldset className="stack"><legend>Google口コミへの案内</legend><input type="hidden" name="googleReviewMode" value={mode}/><div className="row" role="group" aria-label="Google口コミへの案内方法">{([['disabled','使用しない'],['all','全回答者へ表示'],['score','評価条件を満たした人だけ表示']] as const).map(([value,label])=><button type="button" className={mode===value?'btn':'btn secondary'} key={value} aria-pressed={mode===value} disabled={value==='score'&&!scores.length} onClick={()=>changeMode(value)}>{mode===value?'✓ ':''}{label}</button>)}</div></fieldset>
    ...
    {mode==='score'&&<section className="completion-rule stack"><div><h3>口コミを表示する評価条件</h3><p className="muted">「すべての評価項目が基準点以上」は全項目を満たした場合だけ表示します。「質問ごとに基準点を設定」は、どれか1つの質問が設定点以上なら、ほかの回答点に関係なくGoogle口コミをご案内します。</p></div>
