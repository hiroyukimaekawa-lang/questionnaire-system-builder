...
export function LiveSurveyPreview({name,version,onEdit,className=''}:{name:string;version:SurveyVersion;onEdit?:(target:string)=>void;className?:string}){
  ...
  const panel=<div className="live-preview-shell"><div className="preview-toolbar" role="tablist" aria-label="プレビュー表示"><button type="button" className={view==='survey'?'active':''} onClick={()=>setView('survey')}>① 回答画面</button><button type="button" className={view==='thanks'?'active':''} onClick={()=>setView('thanks')}>② サンクス</button><button type="button" className={view==='review'?'active':''} onClick={()=>setView('review')}>③ 口コミ遷移</button></div><div className="admin-phone-frame">{view==='survey'?<SurveyRenderer name={name} slug="preview" version={version} preview onEditTarget={edit}/>:<ThanksPanel key={view} slug="preview" text={config.completionText} reviewUrl={config.googleReviewUrl} reviewMode={view==='thanks'?'disabled':reviewMode} primaryColor={config.primaryColor} reviewPromptText={config.googleReviewPromptText} previewCompletion={{reviewEligible:view==='review',comment:view==='review'&&config.reviewTextQuestionId!==null?'こちらに回答者が入力した感想が表示されます。':''}} onEditTarget={edit}/>}</div><p className="preview-hint">①回答画面 → ②サンクス → ③口コミ遷移の順に確認できます。プレビュー内の文章をクリックすると対応する編集欄へ移動します。使用しない文章は編集画面で非表示にできます。</p></div>;
