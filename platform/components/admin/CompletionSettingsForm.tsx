...
export function CompletionSettingsForm({surveyId,versionId,config,questions,onChange}:{surveyId:string;versionId:string;config:SurveyConfig;questions:SurveyQuestion[];onChange?:(patch:Partial<SurveyConfig>)=>void}) {
 ...
 <input type="hidden" name="googleReviewRule" value={JSON.stringify(review.googleReviewRule??null)}/>
    <details className="stack conditional-settings"><summary><strong>詳細設定：条件別の回答後処理</strong></summary><div className="stack"><div><p className="muted">サンクスメッセージや要フォロー判定を回答条件で変更する高度な設定です。Google口コミの表示条件とは別です。</p></div>
      ...
      <button className="btn secondary" type="button" disabled={!scores.length||rules.length>=MAX_RULES} onClick={addRule}>＋ 完了条件ルールを追加</button>{!scores.length&&<small className="muted">スコア質問を追加すると条件を設定できます。</small>}</div></details>
