...
export function SurveyEditorWorkspace({survey,draft}:{survey:any;draft:SurveyVersion}){
  ...
  const syncForm=(event:React.FormEvent<HTMLElement>)=>{const input=event.target as HTMLInputElement|HTMLTextAreaElement|HTMLSelectElement;if(!input.name)return;if(input.name==='name'){setName(input.value);return;}if(['primaryColor','backgroundColor','secondaryColor','accentColor','heroOverlayColor','heroTextColor','buttonBackground','buttonTextColor','cardBackground','logoBadgeBackground','title','heroTitle','questionFontSize','heroSubtitle','description','introText','anonymousText','completionText','submitLabel','logoUrl','iconUrl','logoMode','heroBackgroundType','themeId','googleReviewMode','googleReviewUrl'].includes(input.name))setConfig(current=>input.name==='submitLabel'?{...current,submitLabel:input.value,buttonLabel:input.value}:{...current,[input.name]:input.name==='questionFontSize'?Number(input.value):input.value});};
  ...
  return <div className="preview-first-layout"><div className="editor-panel" onInput={syncForm} onChange={syncForm}>
    ...
    <section id="publish-settings" className="editor-section card publish-help"><p className="form-kicker">公開設定</p><h2>公開前の最終確認</h2><p className="muted">右のプレビューで本編・サンクス・口コミ導線を確認してから、画面上部の公開ボタンを押してください。保存した下書きだけが公開されます。</p></section>
