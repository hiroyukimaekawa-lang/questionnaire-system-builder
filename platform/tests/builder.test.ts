...
test('業種の過去回答を修正すると質問と業種テーマだけ再計算する',()=>{const before={...completeBase,questions:[{id:'q',type:'text' as const,title:'質問',description:'',required:true,sortOrder:0,settings:{},options:[]}],googleReviewEnabled:true,googleReviewUrl:'https://example.com/review'};const after=engine.applyAnswer(before,'businessType','restaurant');assert.equal(after.businessType,'restaurant');assert.equal(after.template,undefined);assert.equal(after.questions,undefined);assert.equal(after.themeId,'restaurant-clean');assert.equal(after.googleReviewUrl,'https://example.com/review')});
