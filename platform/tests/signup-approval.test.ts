...
test('新規profileはsalesかつinactive、既存NULL profileだけactiveにする',()=>{const migration=source('supabase/migrations/20260904083521_sales_signup_approval.sql');assert.match(migration,/where is_active is null/);assert.match(migration,/alter column is_active set default false/);assert.match(migration,/values\(new\.id,[\s\S]*?'sales',false\)/);assert.doesNotMatch(migration,/delete from|drop table|truncate/i)});
test('RLS helperはactiveなstaffとadminだけを許可する',()=>{const migration=source('supabase/migrations/20260904083521_sales_signup_approval.sql');assert.match(migration,/role in \('admin','sales'\) and is_active = true/);assert.match(migration,/role = 'admin' and is_active = true/)});
...
test('inactiveのadmin layoutは管理データ取得前に利用停止中を返す',()=>{const layout=source('app/admin/layout.tsx');assert.match(layout,/if\(!profile\.is_active\)return/);assert.match(layout,/アカウント利用停止中/);assert.ok(layout.indexOf('if(!profile.is_active)return')<layout.indexOf('Promise.allSettled([getAdminSurveys()'))});
