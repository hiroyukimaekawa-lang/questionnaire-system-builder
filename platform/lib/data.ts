...
export async function getSurveyAccess(id:string){const supabase=await createClient();const [{data:canView},{data:canEdit},{data:canManageMembers}]=await Promise.all([supabase.rpc('can_view_survey',{p_survey_id:id}),supabase.rpc('can_edit_survey',{p_survey_id:id}),supabase.rpc('can_manage_survey_members',{p_survey_id:id})]);return {canView:Boolean(canView),canEdit:Boolean(canEdit),canManageMembers:Boolean(canManageMembers)};}
...
export async function getPublicSurvey(slug:string){ const supabase=createPublicClient(); const {data:survey}=await supabase.from('surveys').select('id,name,slug,industry,status,current_published_version_id').eq('slug',slug).eq('status','published').maybeSingle(); if(!survey?.current_published_version_id)return null; const {data:version,error}=await supabase.from('survey_versions').select('*,questions(*,question_options(*))').eq('id',survey.current_published_version_id).single(); return !error&&version?{survey,version:mapVersion(version)}:null; }
