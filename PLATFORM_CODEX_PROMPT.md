...
店舗ごとにBuilderからモード・対象質問・閾値・AND / ORを設定し、回答送信時に条件を判定してください。口コミURLが設定され、モードの表示条件を満たす場合にCTAを表示してください。
...
- Google口コミCTAが `disabled` / `all` / `score` に従い、scoreでは質問単位の閾値・gte / lte / eq互換・複数条件のAND / ORを正しく判定する
- 回答保存
- CSV出力
- RLS
